...
const handleResetSettings = () => {
  ...
  accessibility.updateSetting('navigationVoice', true)
}
...
.vi-mode .a11y-fab { width: 64px; height: 64px; font-size: 24px; bottom: max(32px, env(safe-area-inset-bottom)); right: 32px; }
...
.vi-mode .swap-btn { width: 56px; height: 56px; font-size: 18px; }
...
@media (max-width: 768px) {
  .menu-toggle-fixed { top: max(16px, env(safe-area-inset-top)); left: 16px; }
  ...
  .a11y-fab { bottom: max(20px, env(safe-area-inset-bottom)); right: 20px; }
  .hero-content { gap: 16px; }
}

@media (max-width: 480px) {
  .menu-toggle-fixed { top: max(14px, env(safe-area-inset-top)); left: 14px; width: 40px; height: 40px; font-size: 14px; }
...
@media (hover: none) and (pointer: coarse) {
  ...
  .swap-btn:active { background: var(--accent-dim); color: var(--accent); }
}
</style>