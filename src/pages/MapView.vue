<template>
  <div class="app" :class="[theme, { 'vi-mode': accessibility.settings.visuallyImpaired, 'high-contrast': accessibility.settings.highContrast, 'nav-active': navigationActive }]">
    ...
    <aside v-if="!navigationActive" class="sidebar" :class="{ open: menuOpen }" role="navigation">
      <div class="logo">
      ...
      <div class="sidebar-content">
        ...
        <div v-if="currentMode === 'route'" class="route-panel">
          ...
          <button class="primary-btn" @click="calculateRoute" :disabled="loading">
            <i class="fa-solid fa-diamond-turn-right"></i> Rota Hesapla
          </button>

          <button v-if="routeInfoVisible && !navigationActive" class="start-nav-btn" @click="startNavigation" aria-label="Navigasyonu başlat">
    ...
    <div v-if="navigationActive" class="nav-hud" :class="theme">
      <div class="nav-top-bar">
        ...
        <div class="nav-dest-info">
          <div class="nav-dest-label">Hedefe</div>
...
const startNavigation = () => {
  ...
  accessibility.speakNavigation('Navigasyon başladı. İyi yolculuklar!')
}
...
const stopNavigation = () => {
  ...
  accessibility.speakNavigation('Navigasyon durduruldu.')
}
...
const updateCurrentStep = (stepIndex) => {
  ...
  navCurrentStep.text = step.text || 'İlerleyin'
  ...
  if (nextStep) {
    ...
    navNextStep.text = nextStep.text || ''
...
.start-nav-btn:hover { opacity: 0.9; transform: translateY(-1px); }
.start-nav-btn i { font-size: 16px; }
...
.option-row { display: flex; align-items: center; gap: 10px; cursor: pointer; font-size: 13px; font-weight: 500; color: var(--text); min-height: 36px; }
...
.result-item { background: var(--input-bg); border: 1px solid var(--border); border-radius: 12px; padding: 12px; cursor: pointer; transition: all 0.2s; touch-action: manipulation; }
...
.map-container.nav-mode { height: 100vh; }
...
.cancel-pick { background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); color: #ef4444; padding: 6px 14px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; font-family: 'DM Sans', sans-serif; margin-left: 8px; touch-action: manipulation; }
...
.nav-stop-btn:hover { background: rgba(239,68,68,0.35); }
...
.nav-voice-btn.muted { color: rgba(255,255,255,0.4); border-color: rgba(255,255,255,0.1); }
.nav-voice-btn:hover { background: rgba(255,255,255,0.18); }
...
.nav-arrow-icon.arrival { background: #22c55e; }
.nav-arrow-icon.turn-left, .nav-arrow-icon.sharp-left { background: #f59e0b; }
.nav-arrow-icon.turn-right, .nav-arrow-icon.sharp-right { background: #f59e0b; }
.nav-arrow-icon.uturn { background: #ef4444; }
.nav-arrow-icon.roundabout { background: #8b5cf6; }
...
.nav-wc-badge {
  ...
  height: 36px;
  border-radius: 50%;
  background: rgba(22,166,103,0.2);
...
@media (max-width: 768px) {
  ...
  .nav-arrow-panel { padding: 16px 16px; gap: 14px; }
  .nav-arrow-icon { width: 60px; height: 60px; font-size: 26px; border-radius: 16px; }
  .nav-step-dist { font-size: 24px; }
  .nav-step-text { font-size: 14px; }
}

@media (max-width: 480px) {
  .menu-toggle-fixed { top: 12px; left: 12px; width: 40px; height: 40px; font-size: 14px; }
  .sidebar.open { width: 92%; max-width: 300px; }
  .nav-top-bar { padding: 10px 12px; }
  .nav-arrow-panel { padding: 12px 14px; gap: 12px; }
  .nav-arrow-icon { width: 54px; height: 54px; font-size: 22px; }
  .nav-step-dist { font-size: 20px; }
  .nav-step-text { font-size: 13px; }
  .start-nav-btn { font-size: 14px; padding: 12px; }
}

@media (hover: none) and (pointer: coarse) {
  .menu-toggle-fixed, .mode-tab, .primary-btn, .pick-btn, .nav-item, .result-item, .a11y-fab, .start-nav-btn, .nav-stop-btn, .nav-voice-btn { -webkit-tap-highlight-color: transparent; }
  .result-item:active { background: var(--accent-dim); border-color: var(--accent); }
  .start-nav-btn:active { transform: scale(0.97); }
}
