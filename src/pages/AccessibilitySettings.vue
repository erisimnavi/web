    ...
    <div v-if="open" class="a11y-backdrop" @click.self="$emit('close')" role="dialog" aria-modal="true" aria-label="Erişilebilirlik Ayarları">
      <div class="a11y-panel" :class="theme">
        ...
        <div class="a11y-body">
          ...
          <div class="a11y-section">
            <div class="a11y-section-title">Navigasyon Sesi</div>
            <div class="a11y-toggle-row" :class="{ active: settings.navigationVoice }">
              <div class="a11y-toggle-info">
                <i class="fa-solid fa-location-arrow"></i>
                <div>
                  <div class="a11y-toggle-label">Navigasyon Sesi</div>
                  <div class="a11y-toggle-desc">Navigasyon sırasında yön, mesafe ve eğim bilgilerini sesli söyler (tüm kullanıcılar için)</div>
                </div>
              </div>
              <div
                class="a11y-switch"
                :class="{ on: settings.navigationVoice ?? true }"
                @click="set('navigationVoice', !(settings.navigationVoice ?? true))"
                role="switch"
                :aria-checked="settings.navigationVoice ?? true"
                tabindex="0"
                @keydown.enter="set('navigationVoice', !(settings.navigationVoice ?? true))"
                @keydown.space.prevent="set('navigationVoice', !(settings.navigationVoice ?? true))"
              >
                <div class="a11y-switch-knob"></div>
              </div>
            </div>
          ...
          <div class="a11y-section" :class="{ disabled: !settings.visuallyImpaired }">
            <div class="a11y-section-title">Sesli Okuma (Görme Engelli)</div>
            <label class="a11y-check-row">
...
defineProps({
  ...
  speak: { type: Function, default: null },
  stopSpeaking: { type: Function, default: null }
})
...
@media (max-width: 480px) {
  .a11y-panel { border-radius: 16px; max-height: 95vh; }
  .a11y-header { padding: 16px 18px; }
  .a11y-body { padding: 16px 18px; gap: 16px; }
  .a11y-footer { padding: 12px 18px; }
}
</style>